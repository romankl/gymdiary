...
public class Exercise: Object {
    ...
    // workout name
    dynamic var type = 0
    // workout type (0 = weight, 1 = time/ distance)
    dynamic var builtin = false
    // is it a built in Exercise
    dynamic var bodyGroup = 0
    // TODO: Define!!
    dynamic var comment = ""
    // comment (optional)
    dynamic var lastUsed = NSDate()
    // when was this exercise the last time
    dynamic var used = 0
    // inc each time this one is used
    dynamic var archived = false
    // built in exercises can be archived
...
public class Performance: Object {
    ...
    dynamic var preReps = 0
    // Pre- defined values
    dynamic var preWeight = Double()
    // Pre- defined values
...
public class Workout: Object {
    ...
    let performedExercises = List<PerformanceExerciseMap>()
    // "To many" for the
    dynamic var name = ""
    // name of the workout -> should be the copy of the original workout
    dynamic var startedAt = NSDate()
    // date when the workout was pe
    dynamic var endedAt = NSDate()
    // when the workout was ended
    dynamic var active = true
    // is the workout still running?
    dynamic var comment = ""
    // optional comment
    dynamic var bodyWeight = Double()
    // optional bodyweight
    dynamic var basedOnWorkout: WorkoutRoutine?
    // original workout / optional in case that its a freeform workout
    dynamic var totalReps = 0
    dynamic var totalWeight = Double()
    dynamic var totalDistance = Double()
    dynamic var totalRunningTime = Double()
    // Defines the total time of all running exercises - not the total time of the workout
